...
{
	//Locks down the Ability list to modifications until this scope/function is done as Ability states can change while
	//this code is running, if modifications are made when locked they will happen once this code finishes
	FScopedAbilityListLock ActiveScopeLock(*this);
	for (const FGameplayAbilitySpec& AbilitySpec : GetActivatableAbilities())
	{
		if (!Delegate.ExecuteIfBound(AbilitySpec))
		{
			// __FUNCTION__ is to get the name of the function
...
void UAuraAbilitySystemComponent::ServerEquipAbility_Implementation(const FGameplayTag& AbilityTag, const FGameplayTag& Slot)
{
	if (FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag))
	{
		...
		if (bStatusValid)
		{
			// Remove this InputTag (Slot) any Ability that has it.
			ClearAbilitiesOfSlot(Slot);
			// Clear this Ability Slot, just in case it's a different sloty
